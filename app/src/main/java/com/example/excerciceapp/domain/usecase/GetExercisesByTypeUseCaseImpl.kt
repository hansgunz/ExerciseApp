...
class GetExercisesByTypeUseCaseImpl @Inject constructor(
    private val remoteRepository: RemoteRepository,
    private val errorHandler: ErrorHandler
): GetExercisesByTypeUseCase {
    private val TAG = this::class.simpleName

    override fun invoke(type: String): Flow<Result<List<Exercise>, DataError.Network>> = flow {
        try {
            emit(Result.Loading())
            val exercises =
                remoteRepository.fetchExercisesByType(type).map { it.toExercise() }
